...
uint8_t auth_code[8] = {27, 13, 157, 34, 53, 37, 1, 7};  //special code for each device
...
{  
  ...
  prepare_number_to_data_msg((get_motion() ? 1 : 0), &_rest_of_value, &_value);
  ...
  prepare_number_to_data_msg(get_light(), &_rest_of_value, &_value);
...
void send_voltage(float voltage)
{
  ...
  byte data_to_send[5];
  int size_of_msg = sizeof(data_to_send) + 4;
  byte _rest_of_voltage;
  long _voltage;

  prepare_number_to_data_msg(voltage, &_rest_of_voltage, &_voltage);
  
  convert_number_to_array_on_position(data_to_send, 0, 2, 65535);
  convert_number_to_array_on_position(data_to_send, 2, 2, _voltage);
  convert_number_to_array_on_position(data_to_send, 4, 1, _rest_of_voltage);
...
bool get_motion()
{
  return (digitalRead(pir) ? true : false);
}
       ...
       case 6 : //have to send data MSG about devices I have
          {
            ...
            int type_of_device_1 = 11;
            int type_of_device_2 = 12;
...
void set_command()
{
  ...
  for (int i = 0; i < number_of_cmds; i++)
  {
    ...
    push_cmd_to_buffer(convert_byte_to_int(packet_buffer, (5 + offset), 2), convert_byte_to_int(packet_buffer, (7 + offset), 2), packet_buffer[9 + offset], convert_byte_to_int(packet_buffer, (10 + offset), 1));
