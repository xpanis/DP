...
//----------Start of define----------
#define ss_gas_sensor 5
#define ss_air_sensor 4
...
uint8_t auth_code[8] = {163, 158, 103, 123, 98, 79, 48, 156};  //special code for each device
...
void set_command();
void stop_function();
void gas_enable_air_disable();
void gas_disable_air_enable();
byte get_air();
byte get_gas();
...
{  
  ...
  prepare_number_to_data_msg(get_gas(), &_rest_of_value, &_value);
  ...
  prepare_number_to_data_msg(get_air(), &_rest_of_value, &_value);
...
void sensors_and_actuators_init()
{
  pinMode(ss_gas_sensor, OUTPUT);
  pinMode(ss_air_sensor, OUTPUT);
}


void gas_enable_air_disable()
{
  digitalWrite(ss_air_sensor, LOW);
  digitalWrite(ss_gas_sensor, HIGH);
}
...
void gas_disable_air_enable()
{
  digitalWrite(ss_gas_sensor, LOW);
  digitalWrite(ss_air_sensor, HIGH);
}
       ...
       case 6 : //have to send data MSG about devices I have
          {
            ...
            int type_of_device_1 = 9;
            int type_of_device_2 = 10;
