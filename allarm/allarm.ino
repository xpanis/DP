...
uint8_t auth_code[8] = {1, 5, 15, 65, 245, 76, 115, 111};  //special code for each device
...
void send_data_allarm_state(bool state);
...
void allarm(bool state)
{
  ...
  {
    digitalWrite(5, HIGH);
    send_data_allarm_state(true);
  }
  else
  {
    digitalWrite(5, LOW);
    send_data_allarm_state(false);
  }
}
  ...
  for (int i = 0; i < 10; i++)
  {
    if ((convert_byte_to_int(sheduling_table[i], 5, 2)) <= minutes)
    {
      switch(convert_byte_to_int(sheduling_table[i], 0, 2)) // must be actuator
      {
         case 1:
         {
            allarm(((byte) get_float_from_cmd_format(convert_byte_to_int(sheduling_table[i], 2, 2), (byte) convert_byte_to_int(sheduling_table[i], 4, 1))) ? true : false);
            break;
         }
         default:
         {
