...
bool kettle1_state = false;
bool kettle2_state = false;
byte kettle1_times = 0;
byte kettle2_times = 0;
//----------End of Variables----------
...
void timer()
{
  ...
  if (last_minute < minutes)
  {
   ...
   if (kettle1_state)
   {
    ...
    if (kettle1_times >= 2)
    {
      change_kettle_state(0, 1);
      send_data_kettle_off(1);
      kettle1_times = 0;
    }
   }
   ...
   if (kettle2_state)
   {
    ...
    if (kettle2_times >= 2)
    {
      change_kettle_state(0, 2);
      send_data_kettle_off(2);
      kettle2_times = 0;
    }
   }
...
void sensors_and_actuators_init()
{
  pinMode(kettle1, OUTPUT);
  pinMode(kettle2, OUTPUT);
}
...
void change_kettle_state(byte state, byte device)
{
  switch(device)
  {
     case 1 :
     {
      ...
      {
        kettle1_state = true;
        digitalWrite(kettle1, HIGH);
      }
      else
      {
        kettle1_state = false;
        digitalWrite(kettle1, LOW);
      }
      break;
     }
     case 2 :
     {
      ...
      {
        kettle2_state = true;
        digitalWrite(kettle2, HIGH);
      }
      else
      {
        kettle2_state = false;
        digitalWrite(kettle2, LOW);
      }
      break;
     }
  }
